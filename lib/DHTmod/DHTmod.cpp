...
void DHTmod::setup(uint8_t pin, DHT_MODEL_t model)
{
  DHTmod::pin = pin;
  DHTmod::model = model;
  DHTmod::resetTimer(); // Make sure we do read the sensor in the next readSensor()
...
void DHTmod::resetTimer()
{
  DHTmod::lastReadTime = millis() - 3000;
}

float DHTmod::getHumidity()
{
  readSensor();
  return humidity;
}

float DHTmod::getTemperature()
{
  readSensor();
  return temperature;
}
...
const char* DHTmod::getStatusString()
{
  switch ( error ) {
    case DHTmod::ERROR_TIMEOUT:
      return "TIMEOUT";

    case DHTmod::ERROR_CHECKSUM:
      return "CHECKSUM";
...
char const P_OK[]       PROGMEM = "OK";
char const P_TIMEOUT[]  PROGMEM = "TIMEOUT";
char const P_CHECKSUM[] PROGMEM = "CHECKSUM";
